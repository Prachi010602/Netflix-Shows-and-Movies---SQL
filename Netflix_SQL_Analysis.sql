...
-- What were the top 5 shows according to IMDB score? 
SELECT title, 
type, 
imdb_score
FROM netflix.titles
...
GROUP BY age_certification
ORDER BY certification_count ASC
LIMIT 3;
...
-- Who were the top 5 directors that directed the most movies/shows? 
...
-- Finding the titles and  directors of movies released on or after 1960
...
-- Titles and Directors of movies with high IMDB scores (>6) and high TMDB popularity scores (>15) 
SELECT t.title, 
c.name AS director
FROM netflix.titles AS t
JOIN netflix.credits AS c 
